...
void loadStudents(int key)
{
  ...
  if (fp){
    ...
    int match = sscanf(fp, "%s %s %s %s", b1, b2, b3, b4);
    if(match == 4){
      if (key != 0){
        caeserDecrypt(b1, key);
        ceaserDecrypt(b2, key);
        ceaserDecrypt(b3, key);
        ceaserDecrypt(b4, key);
      }
    }
    int age;
    int longid;
    ...
    createStudent(b1, b2, age, longid);
}
